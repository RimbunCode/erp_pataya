...
- [x] T00 `currencyCode` terima string ATAU object di `useCurrency`
...
- [x] T01 Migrasi `variableTokenUtils.js` + update `variableTokenUtils.test.js`
...
- [x] T02 Migrasi `formatValue` sisa: `initHandlebar.js`, `gjsRelationsTable.js`, `PrintPreview.jsx`
...
- [x] T08b Table2 Cell — case `number`/`currency` via `formatNumber`
...
- [x] T09 Native input (9 migrasi; UnitInputField/InputGroupInput dikecualikan — komponen composite styled, per keputusan user) + handler string→float
...
- [x] T10 Hapus kode lama (HANYA jika grep referensi = 0) — termasuk migrasi WorkOrders/Form.jsx yg luput dari task modul (terdeteksi grep guard)
...
- [x] T12 (CHECKPOINT — stop & konfirmasi user) Build + full test
