<?php

return [
    'title'              => 'Items',
    'add'                => 'Add Item',
    'items'              => 'Items',
    'new'                => 'New Item',
    'delete'             => 'Delete Item',
    'delete.description' => 'Are you sure you want to delete this item?',
    'delete.confirm'     => 'Delete',
    'cancel'             => 'Cancel',
    'item'               => 'Item',
    'menu'               => [
        'details'      => 'Details',
        'variants'     => 'Variants',
        'uom'          => 'Units of Measure',
        'stock_levels' => 'Stock Levels',
        'barcodes'     => 'Barcodes',
    ],
    'columns' => [
        'sku'                      => 'SKU',
        'total_stock'              => 'Total Stock',
        'code'                     => 'Part No.',
        'name'                     => 'Name',
        'description'              => 'Description',
        'category'                 => 'Category',
        'category.placeholder'     => 'Select a category',
        'default_unit'             => 'Default Unit',
        'default_unit.placeholder' => 'Select a unit',
        'stock_minimum'            => 'Stock Minimum',
        'is_disabled'              => 'Status',
        'is_disabled.placeholder'  => 'Select a status',
        'is_disabled.parse'        => [
            'false' => 'Enabled',
            'true'  => 'Disabled',
        ],
        'format_variant'             => 'Format Variant',
        'format_variant.placeholder' => 'Using "{" to display attributes',
        'attribute'                  => 'Attribute',
        'attribute.placeholder'      => 'Select an attribute',
        'attribute_values'           => 'Values',
        'allow_alternative_item'     => 'Allow Alternative Item',
        'image'                      => 'Image',
        'image.upload'               => 'Upload Image',
        'image.remove'               => 'Remove Image',
        'barcodes.columns'           => [
            'barcode' => 'Barcode',
            'unit'    => 'Unit',
        ],
        'conversion_factor'      => 'Conversion Factor',
        'is_stock_item'          => 'Is Stock Item',
        'is_stock_item.tooltip'  => 'This value follows the selected Category type and the Fixed Asset status, and cannot be changed manually.',
        'is_fixed_asset'         => 'Is Fixed Asset',
        'is_fixed_asset.tooltip' => 'A service category item cannot be marked as a fixed asset.',
    ],
    'variant_of' => 'Variant of',
    'variants'   => [],
];
