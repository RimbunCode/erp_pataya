...
return new class extends Migration
{
    ...
    public function up(): void {
        ...
        // kotor (quantity * rate), diskon dokumen disimpan terpisah di kolom ini,
        // bukan menimpa basic_amount seperti pola PurchaseOrderItem/SalesOrderItem.
        Schema::table('purchase_invoice_items', function (Blueprint $table) {
            $table->double('discount_amount')->default(0)->after('basic_amount');
        });

        // amount -> tax_amount -> dpp_amount saling mereferensikan berurutan --
        // harus di-drop dari yang paling bergantung dulu (SQLite table-rebuild per
        // panggilan Schema::table(), gagal jika masih ada generated column lain yang
        // mereferensikan kolom yang sedang di-drop).
        Schema::table('purchase_invoice_items', function (Blueprint $table) {
        ...
        Schema::table('purchase_invoice_items', function (Blueprint $table) {
            $table->double('amount')->storedAs('(basic_amount - discount_amount) + tax_amount')->after('tax_amount');
        });
    }
