...
export default function CheckoutModal({
  ...
  onClose,
  onSuccess,
  submitRoute = route("student.enroll"),
  submitPayload,
  title = "Checkout",
  label = "Checkout",
  ctaText = "Send",
  successTitle = "Bukti Terkirim!",
  successDescription = "Pendaftaran sedang diverifikasi admin. Proses 1×24 jam kerja.",
  successBadge = "⏳ Menunggu Verifikasi",
  backText = "Back",
  nextText = "Continue",
  ...
  const handleSubmit = () => {
    ...
    if (typeof submitPayload === "function") {
      submitPayload({ formData, items, total, method, file, note });
    } else {
          ...
          <div className="flex flex-col text-lg font-black text-primary mt-2">
            <span
              className={cn(
                ...
                false && "line-through text-muted-foreground text-sm",
              )}
            >
              {formatRp(total)}
            </span>
