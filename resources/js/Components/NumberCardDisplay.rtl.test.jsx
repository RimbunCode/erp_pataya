...
// {filters}) -> {value, percentage}. BlockDescriptionTooltip di-stub jadi spy
// (pola sama NumberCardBlock/LinkCardBlock.rtl.test.jsx -- logic & render
// aslinya sudah dites terpisah di BlockDescriptionTooltip.test.js). resolveIcon
// & formatNumber dipakai APA ADANYA (real, pure) supaya integrasi sungguhan
// ikut teruji, bukan cuma prop passthrough.
...
// dipanggil di useEffect saat mount dan resolusinya (walau mock instan)
// lanjut di microtask SESUDAH act() sinkron itu selesai. Bungkus render()
// ITU SENDIRI dgn await act(async () => {...}) supaya React menunggu
// microtask .then()/.catch() axios sampai stabil sebelum baris berikutnya
// jalan (lihat pola sama di ChartDisplay.rtl.test.jsx).
const renderSettled = async (props) => {
  let utils;
  await act(async () => {
    utils = render(<NumberCardDisplay {...props} />);
  });
  return utils;
};
...
describe("NumberCardDisplay — state loading & error", () => {
  it("menampilkan 'Memuat data...' selama request pending, hilang setelah selesai", async () => {
    ...
    await act(async () => {
      render(<NumberCardDisplay numberCard={baseNumberCard} />);
    });
  ...
  it("tidak melakukan update state setelah unmount (guard `cancelled`), tidak melempar error", async () => {
    let resolveFetch;
    axiosPost.mockImplementation(
      () =>
        new Promise((resolve) => {
    ...
    let unmount;
    await act(async () => {
      ({ unmount } = render(<NumberCardDisplay numberCard={baseNumberCard} />));
    });

    unmount();

    // Resolve SESUDAH unmount -- guard `cancelled` di useEffect harus
    // mencegah setState pada komponen yang sudah dilepas. Kalau guard hilang,
    // React memunculkan warning "Can't perform a React state update on an
    // unmounted component" (act() akan tetap resolve tanpa melempar, jadi
    // yang divalidasi di sini adalah tidak ada exception yang lolos).
    await expect(
      act(async () => {
