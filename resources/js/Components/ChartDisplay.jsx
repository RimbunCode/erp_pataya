...
function ChartDisplay({ chart, filters = {} }) {
  ...
  const groupByRows = React.useMemo(() => {
    ...
    return data.labels.map((label, i) => ({ period: label, [metricKey]: values[i] ?? 0 }));
  ...
  const chartConfig = React.useMemo(
    () => ({
      [metricKey]: { label: chart.chart_name, color: chart?.color || palette[4] },
    }),
      ...
      case "line":
        return (
          <LineChart accessibilityLayer data={chartData}>
            ...
            <ChartTooltip content={<ChartTooltipContent nameKey="period" />} />
            <Line type="monotone" dataKey={metricKey} stroke={chartConfig[metricKey].color} strokeWidth={3} dot={{ r: 3 }} activeDot={{ r: 5 }} />
          </LineChart>
        );
      case "pie":
      case "donut": {
        ...
        return (
          <PieChart>
            <ChartTooltip content={<ChartTooltipContent nameKey="period" hideLabel />} />
            <Pie
              data={chartData}
              dataKey={metricKey}
              nameKey="period"
              ...
              onMouseEnter={(_, index) => chartData[index]?.period && setActivePeriod(chartData[index].period)}
            >
              {chartData.map((_entry, index) => (
                <Cell key={`cell-${index}`} fill={palette[index % palette.length]} />
              ))}
              {!isDonut && (
                <LabelList dataKey="period" className="fill-background" stroke="none" fontSize={12} />
              )}
              {isDonut && (
                <Label
                  content={({ viewBox }) => {
      ...
      default:
        return (
          <BarChart accessibilityLayer data={chartData}>
            ...
            <XAxis dataKey="period" tickLine tickMargin={10} axisLine />
            <ChartTooltip content={<ChartTooltipContent nameKey="period" />} />
            <Bar dataKey={metricKey} name={chartConfig[metricKey].label} fill={chartConfig[metricKey].color} radius={4} />
          </BarChart>
        );
    }
  };
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        <div className="flex h-40 items-center justify-center text-sm text-destructive">Gagal memuat data.</div>
      ) : isHeatmap ? (
        <HeatmapCalendar data={data ?? {}} color={chart?.color || palette[4]} year={chart?.heatmap_year} />
      ) : (
...
function HeatmapCalendar({ data, color, year }) {
  ...
  const maxCount = Math.max(1, ...Object.values(data).map((v) => Number(v) || 0));
  const cellSize = 12;
  const gap = 3;
  ...
  return (
    <div className="overflow-x-auto">
      <svg width={weeks * (cellSize + gap) + gap} height={7 * (cellSize + gap) + gap} role="img" aria-label={`Heatmap ${targetYear}`}>
        {days.map((day, i) => {
          ...
          const key = Date.UTC(day.getFullYear(), day.getMonth(), day.getDate()) / 1000;
