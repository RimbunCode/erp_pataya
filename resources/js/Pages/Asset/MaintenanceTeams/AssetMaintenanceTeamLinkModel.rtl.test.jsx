...
// LinkModel.jsx (dipakai AssetMaintenanceTeamLinkModel di bawah tangan)
...
// AssetMaintenanceTeamLinkModel -- mock jadi stub kosong sama seperti
...
// Form asli AssetMaintenanceTeam (./Form.jsx, co-located) meng-import
// BranchLinkModel, UserLinkModel & FormTable -- dependency berat yang tidak
// relevan untuk test wiring ini. FormPageDialog di atas sudah di-mock null
// sehingga prop `form` TIDAK PERNAH benar-benar dirender, tapi modulnya
// tetap ikut ter-load lewat static import AssetMaintenanceTeamLinkModel.jsx
// kalau tidak di-mock di sini juga.
vi.mock("./Form", () => ({
  default: () => null,
}));
...
import AssetMaintenanceTeamLinkModel from "./AssetMaintenanceTeamLinkModel";
...
// tanpa di-await -- bungkus render() ITU SENDIRI dalam
...
describe("AssetMaintenanceTeamLinkModel", () => {
  beforeEach(() => {
    axiosPost.mockReset();
    axiosPost.mockResolvedValue({
      data: {
        data: [
          {
            id: 1,
            templateLink: ":team_name",
            team_name: "Tim Maintenance A",
            // FQCN Laravel asli App\Models\Asset\Maintenance\AssetMaintenanceTeam
            // (app/Models/Asset/Maintenance/AssetMaintenanceTeam.php) --
            // literal string JS di sini WAJIB double-backslash supaya
            // menghasilkan satu backslash sungguhan per separator, dibanding
            // literal atribut JSX di AssetMaintenanceTeamLinkModel.jsx sendiri
            // (baris 16) yang cukup satu backslash karena string literal JSX
            // TIDAK memproses escape sequence ala JS biasa (beda dari file .js
            // biasa) -- keduanya berakhir sbg string runtime yang sama.
            thisModel: "App\\Models\\Asset\\Maintenance\\AssetMaintenanceTeam",
          },
          {
            id: 2,
            templateLink: ":team_name",
            team_name: "Tim Maintenance B",
            thisModel: "App\\Models\\Asset\\Maintenance\\AssetMaintenanceTeam",
          },
        ],
        total: 2,
  ...
  it("render input kosong tanpa value", async () => {
    await render(<AssetMaintenanceTeamLinkModel />);
    expect(screen.getByRole("textbox")).toHaveValue("");
  });

  it("render label value terpilih via convertTemplateLink (:team_name)", async () => {
    await render(
      <AssetMaintenanceTeamLinkModel
        value={{ templateLink: ":team_name", team_name: "Tim Aset" }}
      />,
    );
    expect(screen.getByRole("textbox")).toHaveValue("Tim Aset");
  });

  it("placeholder diteruskan ke input", async () => {
    await render(
      <AssetMaintenanceTeamLinkModel placeholder="Pilih tim maintenance..." />,
    );
    expect(
      screen.getByPlaceholderText("Pilih tim maintenance..."),
    ).toBeInTheDocument();
  });
  ...
  it("ref diteruskan ke handle imperatif Input (bukan node DOM mentah)", async () => {
    ...
    // spesifik AssetMaintenanceTeamLinkModel), tapi tetap perlu dikunci di
    // sini supaya wiring `ref` dari AssetMaintenanceTeamLinkModel ->
    // LinkModel -> Input tidak diam-diam berubah.
    const ref = { current: null };
    await render(<AssetMaintenanceTeamLinkModel ref={ref} />);
    expect(typeof ref.current?.focus).toBe("function");
    expect(() => ref.current.focus()).not.toThrow();
  });

  it("disabled mencegah input diedit", async () => {
    await render(<AssetMaintenanceTeamLinkModel disabled />);
    expect(screen.getByRole("textbox")).toBeDisabled();
  });

  it("mengetik memicu request axios dengan model AssetMaintenanceTeam", async () => {
    const user = userEvent.setup({ delay: null });
    await render(<AssetMaintenanceTeamLinkModel />);

    await act(async () => {
      await user.type(screen.getByRole("textbox"), "Tim");
    });
      ...
      () => {
        expect(axiosPost).toHaveBeenCalledWith(
          ...
          expect.objectContaining({
            // Literal atribut JSX di AssetMaintenanceTeamLinkModel.jsx baris
            // 16 (model="App\Models\Asset\Maintenance\AssetMaintenanceTeam")
            // memakai satu backslash -- berbeda dari string literal JS biasa,
            // JSX TIDAK memproses escape sequence pada literal atribut string,
            // jadi backslash-nya tetap utuh saat runtime dan cocok dgn FQCN
            // Laravel asli app/Models/Asset/Maintenance/AssetMaintenanceTeam.php.
  ...
  it("membuka dropdown menampilkan titleDialog (t('asset.maintenance.team.new')) sbg label tombol tambah", async () => {
    const user = userEvent.setup({ delay: null });
    await render(<AssetMaintenanceTeamLinkModel />);
    ...
    expect(
      await screen.findByText("TR:asset.maintenance.team.new"),
    ).toBeInTheDocument();
  });
  ...
  it("memilih opsi dari daftar hasil memanggil onValueChange", async () => {
    ...
    await render(
      <AssetMaintenanceTeamLinkModel onValueChange={onValueChange} />,
    );
    ...
    await waitFor(
      () => {
        expect(axiosPost).toHaveBeenCalledWith(
          "model",
          expect.objectContaining({ search: "Tim" }),
        );
      },
      { timeout: 3000 },
    );
    ...
    expect(onValueChange).toHaveBeenCalledWith(
      expect.objectContaining({ id: 1, team_name: "Tim Maintenance A" }),
    );
  });

  it("opsi dengan thisModel model lain (bukan AssetMaintenanceTeam) ditolak validate() -- onValueChange TIDAK terpanggil", async () => {
    // validate() di linkModelUtils.js membandingkan value.thisModel === model
    // secara ketat. AssetMaintenanceTeamLinkModel mengunci model ke
    // AssetMaintenanceTeam, jadi opsi yang (secara data cacat/salah) membawa
    // thisModel model lain harus ditolak secara silent (tanpa error) -- bukan
    // celah keamanan karena payload pencarian sendiri sudah scoped ke model
    // AssetMaintenanceTeam, tapi defense-in-depth ini tetap perlu dikunci
    // lewat test.
    axiosPost.mockResolvedValue({
      data: {
        data: [
  ...
  it("prop tambahan (mis. model) via spread menimpa konfigurasi default AssetMaintenanceTeam", async () => {
    // AssetMaintenanceTeamLinkModel men-spread {...props} SETELAH prop tetap
    // (model, titleDialog, form) -- di JSX, atribut yang ditulis belakangan
    // menang. Jadi caller BISA menimpa `model` bawaan AssetMaintenanceTeam
    ...
    const user = userEvent.setup({ delay: null });
    await render(<AssetMaintenanceTeamLinkModel model="AppModelsOverride" />);
