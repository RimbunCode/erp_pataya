...
import PurchaseOrderLinkModel from "./PurchaseOrderLinkModel";
...
describe("PurchaseOrderLinkModel", () => {
  beforeEach(() => {
    axiosPost.mockReset();
  ...
  it("render input kosong tanpa value", async () => {
    await render(<PurchaseOrderLinkModel />);
  ...
  it("render label value terpilih via convertTemplateLink (:code)", async () => {
    await render(
      <PurchaseOrderLinkModel
        value={{ templateLink: ":code", code: "PO-0099" }}
      />,
    );
    expect(screen.getByRole("textbox")).toHaveValue("PO-0099");
  });

  it("placeholder diteruskan ke input", async () => {
    await render(<PurchaseOrderLinkModel placeholder="Pilih PO..." />);
    expect(screen.getByPlaceholderText("Pilih PO...")).toBeInTheDocument();
  });

  it("ref diteruskan ke handle imperatif Input (bukan node DOM mentah)", async () => {
    const ref = { current: null };
    await render(<PurchaseOrderLinkModel ref={ref} />);
    expect(typeof ref.current?.focus).toBe("function");
    expect(() => ref.current.focus()).not.toThrow();
  });

  it("disabled mencegah input diedit", async () => {
    await render(<PurchaseOrderLinkModel disabled />);
    expect(screen.getByRole("textbox")).toBeDisabled();
  });

  it("mengetik memicu request axios dengan model PurchaseOrder", async () => {
    const user = userEvent.setup({ delay: null });
    await render(<PurchaseOrderLinkModel />);

    await act(async () => {
      await user.type(screen.getByRole("textbox"), "PO-0001");
    });
  ...
  it("membuka dropdown menampilkan titleDialog (t('purchase.purchaseOrder.new')) sbg label tombol tambah", async () => {
    const user = userEvent.setup({ delay: null });
    await render(<PurchaseOrderLinkModel />);
    ...
    expect(
      await screen.findByText("TR:purchase.purchaseOrder.new"),
    ).toBeInTheDocument();
  });
  ...
  it("memilih opsi dari daftar hasil memanggil onValueChange", async () => {
    ...
    await render(<PurchaseOrderLinkModel onValueChange={onValueChange} />);
    ...
    await waitFor(() => {
      expect(axiosPost).toHaveBeenCalledWith(
        "model",
        expect.objectContaining({ search: "PO-0001" }),
      );
    });

    const option = await screen.findByRole("option", { name: "PO-0001" });
    await act(async () => {
      await user.click(option);
    });

    expect(onValueChange).toHaveBeenCalledWith(
      expect.objectContaining({ id: 1, code: "PO-0001" }),
    );
  });

  it("opsi dengan thisModel model lain (bukan PurchaseOrder) ditolak validate() -- onValueChange TIDAK terpanggil", async () => {
    axiosPost.mockResolvedValue({
      data: {
        data: [
          {
  ...
  it("prop tambahan (mis. model) via spread menimpa konfigurasi default PurchaseOrder", async () => {
    const user = userEvent.setup({ delay: null });
    await render(<PurchaseOrderLinkModel model="AppModelsOverride" />);
