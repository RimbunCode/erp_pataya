...
const _validRowCountArb = fc.integer({ min: 1, max: 5 });

/** Generate a row count below minimum (≤ 0). */
const _belowMinRowCountArb = fc.integer({ min: -10, max: 0 });

/** Generate a row count at or above maximum (≥ 5). */
const _atOrAboveMaxRowCountArb = fc.integer({ min: 5, max: 20 });
...
describe("Feature: gjs-table-relation-custom-mode, Property 1: Column Filtering Correctness", () => {
      ...
      fc.property(
        ...
        (relationName, basicCols, manyCols) => {
          ...
          for (const col of manyCols) {
            expect(
              result.some((r) => r.name === col.name && r.type === "relations"),
            ).toBe(false);
          }
        },
      ),
  ...
  it("returns empty array for null/undefined inputs", () => {
    ...
    expect(
      filterRelationColumns([{ name: "doc", type: "doc", columns: [] }], ""),
    ).toEqual([]);
  });
  ...
  it("single-relation (type 'relation') columns are included in result", () => {
    fc.assert(
      fc.property(identArb, identArb, (relationName, colName) => {
        ...
        const result = filterRelationColumns(dataTableColumns, relationName);
        expect(
          result.some((r) => r.name === colName && r.type === "relation"),
        ).toBe(true);
      }),
      { numRuns: 100 },
    );
...
describe("Feature: gjs-table-relation-custom-mode, Property 2: Header Row Count Invariant", () => {
...
describe("Feature: gjs-table-relation-custom-mode, Property 3: Span Validation and Overlap Detection", () => {
  ...
  it("validateSpan rejects overlap with existing cells", () => {
    ...
    const grid = {
      totalRows: 2,
      totalColumns: 2,
      rows: [[{ rowIndex: 0, colIndex: 0, colspan: 1, rowspan: 2 }], []],
    };
...
describe("Feature: gjs-table-relation-custom-mode, Property 4: Body Section Structural Invariant", () => {
