...
// LinkModel.jsx (dipakai QuotationLinkModel di bawah tangan) meng-import
// FormPageDialog dari FormPage.jsx, yang menarik banyak dependency lain
// (Table2, editor, dst) tidak relevan untuk test wiring QuotationLinkModel --
...
// Form asli Quotation (./Form.jsx, co-located) meng-import CustomerLinkModel,
// OpportunityLinkModel, QuotationItems, DatetimePicker, dst -- dependency
// berat yang tidak relevan utk test wiring QuotationLinkModel ini.
...
// static import QuotationLinkModel.jsx kalau tidak di-mock di sini juga.
vi.mock("./Form", () => ({
  default: () => null,
}));
...
import QuotationLinkModel from "./QuotationLinkModel";
...
describe("QuotationLinkModel", () => {
        ...
        data: [
          {
            ...
            templateLink: ":code",
            code: "QTN-0001/26",
            // FQCN Laravel asli App\Models\CRM\Quotation
            // (app/Models/CRM/Quotation.php) -- literal string JS di sini
            // WAJIB double-backslash supaya menghasilkan satu backslash
            // sungguhan per separator, dibanding literal atribut JSX di
            // QuotationLinkModel.jsx sendiri (baris 16) yang cukup satu
            ...
            thisModel: "App\\Models\\CRM\\Quotation",
          },
          {
            id: 2,
            templateLink: ":code",
  ...
  it("render input kosong tanpa value", async () => {
    await render(<QuotationLinkModel />);
  ...
  it("render label value terpilih via convertTemplateLink (:code)", async () => {
    await render(
      <QuotationLinkModel
        value={{ templateLink: ":code", code: "QTN-0009/26" }}
      />,
    );
    expect(screen.getByRole("textbox")).toHaveValue("QTN-0009/26");
  });

  it("placeholder diteruskan ke input", async () => {
    await render(<QuotationLinkModel placeholder="Pilih quotation..." />);
    expect(
      screen.getByPlaceholderText("Pilih quotation..."),
    ).toBeInTheDocument();
  });
  ...
  it("ref diteruskan ke handle imperatif Input (bukan node DOM mentah)", async () => {
    ...
    // spesifik QuotationLinkModel), tapi tetap perlu dikunci di sini supaya
    // wiring `ref` dari QuotationLinkModel -> LinkModel -> Input tidak diam-
    // diam berubah.
    const ref = { current: null };
    await render(<QuotationLinkModel ref={ref} />);
    expect(typeof ref.current?.focus).toBe("function");
    expect(() => ref.current.focus()).not.toThrow();
  });

  it("disabled mencegah input diedit", async () => {
    await render(<QuotationLinkModel disabled />);
    expect(screen.getByRole("textbox")).toBeDisabled();
  });

  it("mengetik memicu request axios dengan model Quotation", async () => {
    const user = userEvent.setup({ delay: null });
    await render(<QuotationLinkModel />);

    await act(async () => {
      await user.type(screen.getByRole("textbox"), "QTN");
    });
    ...
    await waitFor(() => {
      expect(axiosPost).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          // Literal atribut JSX di QuotationLinkModel.jsx baris 16
          // (model="App\Models\CRM\Quotation") memakai satu backslash --
  ...
  it("membuka dropdown menampilkan titleDialog (t('crm.quotation.new')) sbg label tombol tambah", async () => {
    const user = userEvent.setup({ delay: null });
    await render(<QuotationLinkModel />);
    ...
    expect(await screen.findByText("TR:crm.quotation.new")).toBeInTheDocument();
  });

  it("memilih opsi dari daftar hasil memanggil onValueChange", async () => {
    const user = userEvent.setup({ delay: null });
    const onValueChange = vi.fn();
    await render(<QuotationLinkModel onValueChange={onValueChange} />);
    ...
    await waitFor(() => {
      expect(axiosPost).toHaveBeenCalledWith(
        "model",
        expect.objectContaining({ search: "QTN" }),
      );
    });
    ...
    expect(onValueChange).toHaveBeenCalledWith(
      expect.objectContaining({ id: 1, code: "QTN-0001/26" }),
    );
  });

  it("opsi dengan thisModel model lain (bukan Quotation) ditolak validate() -- onValueChange TIDAK terpanggil", async () => {
    // validate() di linkModelUtils.js membandingkan value.thisModel === model
    // secara ketat. QuotationLinkModel mengunci model ke Quotation, jadi
    ...
    // payload pencarian sendiri sudah scoped ke model Quotation, tapi
  ...
  it("prop tambahan (mis. model) via spread menimpa konfigurasi default Quotation", async () => {
    // QuotationLinkModel men-spread {...props} SETELAH prop tetap (model,
    // titleDialog, classNameDialog, form) -- di JSX, atribut yang ditulis
    // belakangan menang. Jadi caller BISA menimpa `model` bawaan Quotation
    ...
    const user = userEvent.setup({ delay: null });
    await render(<QuotationLinkModel model="AppModelsOverride" />);
